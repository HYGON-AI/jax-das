...
miopen_mha = import_from_plugin("rocm", "miopen_mha")

if miopen_mha is not None:
  for _name, _value in miopen_mha.registrations().items():
    xla_client.register_custom_call_target(_name, _value, platform="ROCM")

def _require_miopen_mha():
  if miopen_mha is None:
    raise RuntimeError(
        "jax_rocm plugin does not provide miopen_mha; rebuild and install "
        "jax-rocm plugin wheel with DTK flash attention support.")
  return miopen_mha
...
def convert_mask_type_to_string(mask_type: int) -> str:
  if mask_type == MaskType.NO_MASK.value:
    return "NO_MASK"
  elif mask_type == MaskType.PADDING.value:
    return "PADDING"
  elif mask_type == MaskType.CAUSAL.value:
    return "CAUSAL"
  elif mask_type == MaskType.PADDING_CAUSAL.value:
    return "PADDING_CAUSAL"
  elif mask_type == MaskType.ALIBI.value:
    return "ALIBI"
  else:
    raise ValueError(f"Unexpected mask type: {mask_type}")

def has_padding(mask_type: int) -> bool:
  return mask_type == MaskType.PADDING.value or mask_type == MaskType.PADDING_CAUSAL.value
...
def _dot_product_attention_fwd_rule(
  ...
  #       outputs[1], outputs[0])
  res = (query, key, value, bias, q_seqlen, kv_seqlen, outputs[0])
  return outputs[0], res

def _dot_product_attention_bwd_rule(
    mask, scale, seed, dropout_rate, variadic_args, mask_type, layout,
...
def _dot_product_attention_bwd_impl(
    mask, query, key, value, bias, q_seqlen, kv_seqlen, activation, fwd_output,
...
def _dot_product_attention_fwd_cuda_lowering(
  ...
  operands = [query, key, value, bias, mask]
  mha = _require_miopen_mha()
  if T != S:
    raise ValueError(
        "MIOpen flash attention currently requires query and key/value "
        f"sequence lengths to match, got {T} and {S}.")
  workspace_size = mha.compute_miopen_workspace_reserve_space_sizes(B, T, N, H)
  workspace_shape = (workspace_size,)
  if element_type_to_backend_config_type_mapping(query_type.element_type) == "BF16":
    data_type = 0
  else :
    data_type = 1
  opaque = mha.build_miopen_mha_descriptor(B, T, N, H, data_type, layout, workspace_size)
  ...
  if is_training:
    result_types = [
      ...
      ir.RankedTensorType.get((query_shape[0], query_shape[2], query_shape[1]), ir.F32Type.get()),
    ]
    result_layouts = [output_layout] + default_layouts((query_shape[0], query_shape[2], query_shape[1]))
  else:
    result_types = [
      ir.RankedTensorType.get(output_shape, query_type.element_type),
      ir.RankedTensorType.get(workspace_shape, ir.F32Type.get()),
    ]
    result_layouts = [output_layout] + default_layouts(workspace_shape)
  # create custom call here
  out = mlir.custom_call(
    "miOpen_mha_fwd_kernel",
...
_dot_product_attention_fwd_lower = custom_partitioning(
    _dot_product_attention_fwd_impl, static_argnums=(7, 8, 9, 10, 11, 12, 13, 14))

def _dot_product_attention_fwd_infer_sharding_from_operands(
    mask, scale, seed, dropout_rate, variadic_args, mask_type, layout, sliding_window_length,
...
_dot_product_attention_fwd_p = core.Primitive("miopen_dot_product_attention_fwd")
...
_dot_product_attention_fwd_p_wrapper = core.Primitive(
    "miopen_dot_product_attention_fwd_wrapper"
)
...
_dot_product_attention_bwd_p = core.Primitive("miopen_dot_product_attention_bwd")
...
_dot_product_attention_bwd_p_wrapper = core.Primitive(
    "miopen_dot_product_attention_bwd_wrapper"
)
...
# @functools.partial(jax.custom_vjp, nondiff_argnums=(4, 7, 8, 9, 10, 11, 12, 13, 14))
@functools.partial(jax.custom_jvp, nondiff_argnums=(4, 7, 8, 9, 10, 11, 12, 13, 14))
def _dot_product_attention(query: Array,
  ...
  output = _dot_product_attention_fwd(
      query, key, value, bias, mask, q_seqlen, kv_seqlen, scale=scale,
      seed=seed, dropout_rate=dropout_rate, variadic_args=variadic_args,
      mask_type=maskValue, layout=layout, sliding_window_length=sliding_window_length,
...
_dot_product_attention.defjvp(_dot_product_attention_fwd_rule)
...
def dot_product_attention(query: Array,
  ...
  if has_padding(mask_type.value) and (q_seqlen is None or kv_seqlen is None):
  ...
  if layout == AttentionLayout.BTNH:
    B, T, N, _ = query.shape
    _, S, _, _ = key.shape
  else:
    B, N, T, _ = query.shape
    _, _, S, _ = key.shape
  if bias is None:
    bias = jnp.zeros((B, N, T, S), dtype=query.dtype)
  else:
    bias = jnp.broadcast_to(bias, (B, N, T, S)).astype(query.dtype)
  if mask is None:
    mask = jnp.ones((B, N, T, S), dtype=jnp.int8)
  else:
    mask = jnp.broadcast_to(mask, (B, N, T, S)).astype(jnp.int8)
  if q_seqlen is None:
  ...
  output = _dot_product_attention(
      query, key, value, bias, mask, q_seqlen, kv_seqlen, scale, seed,
      dropout_rate, variadic_args, mask_type.value, layout.value, sliding_window_length,
      9.0)
  return output

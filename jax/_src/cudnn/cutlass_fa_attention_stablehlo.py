...
cutlass_fa = import_from_plugin("rocm", "cutlass_fa")

if cutlass_fa is not None:
  for _name, _value in cutlass_fa.registrations().items():
    xla_client.register_custom_call_target(_name, _value, platform="ROCM")

def _require_cutlass_fa():
  if cutlass_fa is None:
    raise RuntimeError(
        "jax_rocm plugin does not provide cutlass_fa; rebuild and install "
...
def _dot_product_attention_fwd_cuda_lowering(
  ...
  fa = _require_cutlass_fa()
  if T != S:
    raise ValueError(
        "Cutlass flash attention currently requires query and key/value "
        f"sequence lengths to match, got {T} and {S}.")
  workspace_size = fa.compute_cutlass_workspace_reserve_space_sizes(B, T, N, H)
  workspace_shape = (workspace_size,)
  if element_type_to_backend_config_type_mapping(query_type.element_type) == "BF16":
    data_type = 0
  else :
    data_type = 1
  opaque = fa.build_cutlass_fa_descriptor(B, T, N, H, data_type, layout, workspace_size)
...
_dot_product_attention_fwd_p = core.Primitive("cutlass_fa_dot_product_attention_fwd")
...
_dot_product_attention_fwd_p_wrapper = core.Primitive(
    "cutlass_fa_dot_product_attention_fwd_wrapper"
)
...
_dot_product_attention_bwd_p = core.Primitive("cutlass_fa_dot_product_attention_bwd")
...
_dot_product_attention_bwd_p_wrapper = core.Primitive(
    "cutlass_fa_dot_product_attention_bwd_wrapper"
)
